/**
 * Test: Create a bet via script, then accept it from a second wallet derivation.
 * Proves the full contract flow works end-to-end.
 */
import {
    AddressTypes,
    Mnemonic,
...
async function main() {
    ...
    console.log('=== Test Bet Flow ===');
    console.log(`Wallet 0: ${wallet0.p2tr}`);

    const provider = new JSONRpcProvider({ url: RPC_URL, network: NETWORK });
    const tailsAddr = Address.fromString(TAILS_ADDRESS);

    // Step 1: Approve MOTO for Tails contract
    console.log('\n--- Step 1: Approve MOTO ---');
    const motoContract = getContract(MOTO_ADDRESS, OP_20_ABI, provider, NETWORK, wallet0.address);
    const betAmount = 10n * 10n ** 8n; // 10 MOTO
    const approveAmount = 1000n * 10n ** 8n; // Approve 1000 MOTO
    ...
    try {
        ...
        console.log(`  Current allowance: ${Number(currentAllowance) / 1e8} MOTO`);

        if (currentAllowance < betAmount) {
            console.log(`  Approving ${Number(approveAmount) / 1e8} MOTO...`);
            const approveResult = await motoContract.increaseAllowance(tailsAddr, approveAmount);
            ...
            console.log('  Waiting for approval to confirm (~30s)...');
            await sleep(35000);
        } else {
            console.log('  Allowance sufficient, skipping approval');
        }
    } catch (err) {
        console.error('  Approval error:', (err as Error).message);
        throw err;
    }

    // Step 2: Create bet
    console.log('\n--- Step 2: Create Bet (10 MOTO) ---');
    const tails = getContract(TAILS_ADDRESS, TailsAbi, provider, NETWORK, wallet0.address);

    try {
        const createResult = await tails.createBet(betAmount);
    ...
    // Wait for confirmation
    console.log('  Waiting for bet to confirm (~35s)...');
    await sleep(35000);
    ...
    try {
        ...
        console.log(`  Total Volume: ${Number(stats.totalVolume) / 1e8} MOTO`);
        ...
        if (stats.nextBetId > 1n) {
            ...
            console.log(`    Creator: 0x${bet.creator.toString(16)}`);
        }
    } catch (err) {
